...
#                 id_text = f"Motorcycle ID: {vehicle_id}"
...
#                 try:
#                     recording['writer'].release()
#                     print(f"Stopped recording for motorcycle {vehicle_id}")
#                 except:
#                     pass
...
#                 errors.append("Missing input type (rtsp or video)")
#             elif input_config['type'] not in ['rtsp', 'video']:
#                 errors.append("Input type must be 'rtsp' or 'video'")
#             else:
#                 if input_config['type'] == 'rtsp':
#                     if 'rtsp_url' not in input_config:
#                         errors.append("Missing rtsp_url for RTSP input")
#                     elif not input_config['rtsp_url'].strip():
#                         errors.append("rtsp_url cannot be empty")
#                 elif input_config['type'] == 'video':
#                     if 'video_path' not in input_config:
#                         errors.append("Missing video_path for video input")
#                     elif not input_config['video_path'].strip():
#                         errors.append("video_path cannot be empty")
#                     elif not os.path.exists(input_config['video_path']):
#                         errors.append(f"Video file not found: {input_config['video_path']}")
...
#         if input_config['type'] == 'rtsp':
#             print(f"Input Type: RTSP Stream")
#             print(f"RTSP URL: {input_config['rtsp_url']}")
#         else:
#             print(f"Input Type: Video File")
#             print(f"Video Path: {input_config['video_path']}")
...
#         print(f"Video capture started: {self.video_path}")
...
#                     self.current_frame_num = self.frame_count
...
#         print("Getting frame from input source...")
        
#         for _ in range(50):
#             ret, frame, _ = self.capture.read()
...
# class MotorcycleSpeedTracker:
#     """Motorcycle speed tracker with evidence recording and clean UI"""
#     def __init__(self, config_manager):
#         self.config = config_manager
        
#         # Load settings from config
#         input_config = self.config.get_input_config()
#         self.input_type = input_config['type']
        
#         if self.input_type == 'rtsp':
#             self.rtsp_url = input_config['rtsp_url']
#         else:
#             self.video_path = input_config['video_path']
...
#         # Changed from car to motorcycle
#         self.class_names = {3: "Motorcycle"}
...
#         # Initialize capture based on input type
#         if self.input_type == 'rtsp':
#             self.capture = RTSPCapture(self.rtsp_url, buffer_size=buffer_size)
#         else:
#             self.capture = VideoCapture(self.video_path)
...
#                         print(f"RECORDING STARTED IMMEDIATELY: Motorcycle {track['id']} - {display_speed:.1f} km/h")
...
#                     cv2.putText(frame, f"Motorcycle {track['id']}", (speed_box_x, speed_box_y - 5), 
#                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
#                     cv2.putText(frame, f"Motorcycle {track['id']}", (status_box_x + 5, status_box_y + 20), 
#                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
#         cv2.putText(frame, f"Motorcycles: {active_tracks}", (150, 30), 
#                    cv2.FONT_HERSHEY_DUPLEX, 0.7, (255, 255, 255), 2)
...
#             cv2.putText(frame, f"VIOLATIONS: {speeding_count}", (320, 55), 
#                        cv2.FONT_HERSHEY_DUPLEX, 0.7, violation_color, 3)
            
#             if flash_time == 0:
#                 warning_overlay = frame.copy()
#                 cv2.rectangle(warning_overlay, (315, 35), (470, 65), (0, 0, 100), -1)
#                 cv2.addWeighted(frame, 0.9, warning_overlay, 0.1, 0, frame)
#         else:
#             cv2.putText(frame, "All motorcycles within limit", (320, 55), 
#                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
...
#         input_text = f"Input: {self.input_type.upper()}"
#         cv2.putText(frame, input_text, (width - 150, 80), 
#                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (150, 150, 255), 1)
        
#         # Violation summary
#         if speeding_count > 0:
#             summary_text = f"WARNING: {speeding_count} motorcycle(s) exceeding speed limit - RECORDING LIVE"
...
#                                      iou=self.iou_threshold, imgsz=640, classes=[3])  # Changed to class 3 for motorcycle
#                 )
                
#                 detections = []
#                 for result in results:
#                     for box in result.boxes:
#                         if int(box.cls) == 3:  # Changed to class 3 for motorcycle
...
#                 cv2.imshow("Motorcycle Speed Tracker with Live Evidence Recording", frame)
...
#         print("Starting Enhanced Motorcycle Speed Tracker with Live Evidence Recording")
#         print(f"Input Type: {self.input_type.upper()}")
...
#         print("  ✓ Dynamic overlay that follows motorcycle movement")
#         print("  ✓ Auto-disappearing frames when motorcycles leave area")
#         print("  ✓ Support for both RTSP streams and video files")
...
# async def main():
#     print("MOTORCYCLE SPEED TRACKER WITH LIVE EVIDENCE RECORDING")
#     print("Configurable via config.yaml")
#     print("Auto-records evidence with live speed tracking")
#     print("Supports both RTSP streams and video files")
#     print()
...
#         input_config = config_manager.get_input_config()
        
#         print("Connecting to input source...")
#         if input_config['type'] == 'rtsp':
#             temp_capture = RTSPCapture(input_config['rtsp_url'])
#         else:
#             temp_capture = VideoCapture(input_config['video_path'])
...
#         tracker = MotorcycleSpeedTracker(config_manager)
...
#         print("-" * 40)
#         print("""# Input source configuration - choose either RTSP or video file
# input:
#   type: "rtsp"  # or "video"
#   rtsp_url: "rtsp://localhost:8554/stream1"  # for RTSP
#   # video_path: "path/to/your/video.mp4"  # for video file
...
#   buffer_size: 2""")
#         print("-" * 40)
#         print("\nFor video file input, change config to:")
#         print("""input:
#   type: "video"
