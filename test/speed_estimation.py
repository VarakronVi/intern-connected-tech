...
class EvidenceRecorder:
    ...
    def start_recording(self, violation_info, frame, timestamp, tracks=None):
        ...
        class_name = violation_info.get('class_name', 'Vehicle')
        filename = f"violation_{timestamp_str}_{class_name}_ID{vehicle_id}_{speed:.0f}kmh.mp4"
    ...
    def _add_evidence_overlay(self, frame, violation_info, timestamp, tracks=None):
        ...
        if violation_info and tracks:
            ...
            if vehicle_id in tracks and tracks[vehicle_id]['missed_frames'] == 0:
                ...
                class_name = violation_info.get('class_name', 'Vehicle')
                id_text = f"{class_name} ID: {vehicle_id}"
    ...
    def _log_violation(self, violation_info, filepath):
        ...
        log_entry = {
            ...
            'vehicle_id': violation_info['vehicle_id'],
            'vehicle_type': violation_info.get('class_name', 'Vehicle'),
...
class ConfigManager:
    ...
    def validate_config(self):
        ...
        # ตรวจสอบ input settings
        if 'input' not in self.config:
            errors.append("Missing input settings")
        else:
            input_config = self.config['input']
            if 'type' not in input_config:
                errors.append("Missing type in input settings")
            elif input_config['type'] not in ['rtsp', 'video']:
                errors.append("Input type must be 'rtsp' or 'video'")
            
            if 'source' not in input_config:
                errors.append("Missing source in input settings")
            elif not input_config['source'].strip():
                errors.append("Input source cannot be empty")
        
        # ตรวจสอบ camera settings (สำหรับ backward compatibility)
    ...
    def get_input_source(self):
        """ได้ input source (RTSP URL หรือ video path)"""
        input_config = self.get_input_config()
        return input_config['source']
    
    def get_input_type(self):
        """ได้ประเภท input (rtsp หรือ video)"""
        input_config = self.get_input_config()
        return input_config['type']
    ...
    def print_config_summary(self):
        ...
        print("=" * 40)
        print(f"Input Type: {self.get_input_type()}")
        print(f"Input Source: {self.get_input_source()}")
...
class InputCapture:
    """Universal input capture for RTSP and video files"""
    def __init__(self, source, input_type, buffer_size=1):
        self.source = source
    ...
    def start(self):
        ...
        if not self.cap.isOpened():
            raise RuntimeError(f"Cannot open {self.input_type} source: {self.source}")
            
        actual_fps = self.cap.get(cv2.CAP_PROP_FPS)
        if actual_fps > 0:
            self.fps = actual_fps
            print(f"Source FPS: {self.fps}")
        ...
        print(f"{self.input_type.upper()} capture started: {self.source}")
        ...
        while self.running:
            ...
            if ret:
                ...
                if self.input_type == 'video':
                    # สำหรับไฟล์วิดีโอ ใช้เวลาจริงของไฟล์
                    frame_timestamp = self.start_time + (self.frame_count / self.fps)
                else:
                    # สำหรับ RTSP ใช้เวลาปัจจุบัน
                ...
                if self.input_type == 'video':
                    # เพิ่ม delay สำหรับไฟล์วิดีโอเพื่อให้เล่นด้วยความเร็วปกติ
                    time.sleep(1.0 / self.fps)
            else:
                if self.input_type == 'video':
                    # วิดีโอจบแล้ว เริ่มใหม่
...
class RegionDrawer:
    ...
    def __init__(self, input_capture):
    ...
    def select_region(self):
        ...
        print("Getting frame from source...")
        
        for _ in range(50):
            ret, frame, _ = self.input_capture.read()
            if ret and frame is not None:
                break
            time.sleep(0.033)
        
        if not ret or frame is None:
            raise RuntimeError("Cannot get frame from input source")
...
class VehicleSpeedTracker:
    """Vehicle speed tracker supporting cars and motorcycles with evidence recording"""
    def __init__(self, config_manager):
        self.config = config_manager
        
        # Load settings from config
        self.input_source = self.config.get_input_source()
        self.input_type = self.config.get_input_type()
        ...
        # รองรับทั้งรถยนต์และมอเตอร์ไซค์
        self.class_names = {2: "Car", 3: "Motorcycle"}
        ...
        # Input Capture
        self.input_capture = InputCapture(self.input_source, self.input_type, buffer_size=buffer_size)
    ...
    def update_tracks(self, detections, frame_timestamp, frame):
        ...
        for detection in detections:
            ...
            # ตรวจสอบว่าเป็น class ที่เราต้องการ (รถยนต์หรือมอเตอร์ไซค์)
            if int(cls) in [2, 3] and self.is_point_in_region((center_x, center_y)):
        ...
        if not self.tracks:
            for i, (detection, world_pos) in enumerate(zip(valid_detections, world_centers)):
                x1, y1, x2, y2, conf, cls = detection
                vehicle_class = "Car" if detection[5] == 2 else "Motorcycle"
            ...
            if len(track_ids) > 0 and len(world_centers) > 0:
                ...
                for det_idx in range(len(valid_detections)):
                    if det_idx not in matched_detections:
                        ...
                        vehicle_class = "Car" if valid_detections[det_idx][5] == 2 else "Motorcycle"
            ...
            if track['missed_frames'] == 0:
                ...
                if display_speed > 0 and self.is_speeding(display_speed):
                    ...
                    if violation_key not in self.recorded_violations:
                        ...
                        print(f"RECORDING STARTED IMMEDIATELY: {track['class_name']} {track['id']} - {display_speed:.1f} km/h")
    ...
    def draw_results(self, frame):
        ...
        for track in self.tracks.values():
            if track['missed_frames'] == 0:
                ...
                if display_speed > 0:
                    ...
                    cv2.putText(frame, f"{track['class_name']} {track['id']}", (speed_box_x, speed_box_y - 5), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                    ...
                    cv2.putText(frame, f"{track['class_name']} {track['id']}", (status_box_x + 5, status_box_y + 20), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        ...
        # แสดงจำนวนยานพาหนะแยกตามประเภท
        car_count = len([t for t in self.tracks.values() 
                        if t['missed_frames'] == 0 and t['class_name'] == 'Car'])
        motorcycle_count = len([t for t in self.tracks.values() 
                               if t['missed_frames'] == 0 and t['class_name'] == 'Motorcycle'])

        cv2.putText(frame, f"Cars: {car_count} | Motors: {motorcycle_count}", (150, 30), 
        ...
        source_text = f"Source: {self.input_type.upper()}"
        cv2.putText(frame, source_text, (width - 150, 80), 
    ...
    async def capture_frames_async(self):
        while self.running:
            ret, frame, timestamp = self.input_capture.read()
            if ret and frame is not None:
                try:
            ...
            try:
                ...
                results = await loop.run_in_executor(
                    ...
                    lambda: self.model(frame, verbose=False, conf=self.confidence_threshold, 
                                     iou=self.iou_threshold, imgsz=640, classes=[2, 3])
                )
                ...
                for result in results:
                    for box in result.boxes:
                        cls_id = int(box.cls)
                        if cls_id in [2, 3]:  # รถยนต์หรือมอเตอร์ไซค์
    ...
    async def display_frames_async(self):
        while self.running:
            try:
                ...
                cv2.imshow("Vehicle Speed Tracker - Cars & Motorcycles with Live Evidence Recording", frame)
    ...
    def print_statistics(self):
        print("\nVehicle Speed Monitoring Statistics")
        print("=" * 40)
        ...
        active_vehicles = [t for t in self.tracks.values() if t['missed_frames'] == 0]
        if active_vehicles:
            car_count = len([t for t in active_vehicles if t['class_name'] == 'Car'])
            motorcycle_count = len([t for t in active_vehicles if t['class_name'] == 'Motorcycle'])
            print(f"Active Cars: {car_count}")
            print(f"Active Motorcycles: {motorcycle_count}")
            print("-" * 40)
            
            for track in active_vehicles:
                display_speed, status, _ = self.get_display_speed(track)
                is_violation = display_speed > 0 and self.is_speeding(display_speed)
                
                violation_text = "VIOLATION" if is_violation else "NORMAL"
                
                print(f"{track['class_name']} {track['id']}:")
    ...
    async def run_async(self):
        print("Starting Enhanced Vehicle Speed Tracker with Live Evidence Recording")
        print(f"Input Type: {self.input_type}")
        ...
        print(f"Detecting: Cars and Motorcycles")
        print("\nControls:")
        print("  Q - Quit")
        ...
        print("\nFeatures:")
        print("  ✓ Live speed tracking for Cars and Motorcycles")
        print("  ✓ Support for RTSP streams and video files")
        print("  ✓ Immediate recording when violations detected")
        print("  ✓ Vehicle type classification and display")
        print("  ✓ Separate counting for each vehicle type")
...
async def main():
    print("VEHICLE SPEED TRACKER WITH LIVE EVIDENCE RECORDING")
    print("Supports Cars and Motorcycles - RTSP & Video File Input")
    ...
    try:
        ...
        print("Connecting to input source...")
        input_source = config_manager.get_input_source()
        input_type = config_manager.get_input_type()
        temp_capture = InputCapture(input_source, input_type)
        temp_capture.start()
        time.sleep(3)
        ...
        tracker = VehicleSpeedTracker(config_manager)
        ...
        print("-" * 40)
        print("""# Vehicle Speed Tracker Configuration
camera:
  rtsp_url: "rtsp://localhost:8554/stream1"

input:
  type: "rtsp"  # "rtsp" for live stream or "video" for file
  source: "rtsp://localhost:8554/stream1"  # RTSP URL or video file path
